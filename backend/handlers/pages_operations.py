...
async def handler(request: Request):
    ...
    flow_duration = (
        round(total_duration / len(recent_flows), 2) if recent_flows else 0.0
    )
    ...
    inbound_bytes = sum(
        f.get("inbound_bytes", 0) or f.get("bytes_in", 0) for f in recent_flows
    )
    outbound_bytes = sum(
        f.get("outbound_bytes", 0) or f.get("bytes_out", 0) for f in recent_flows
    )
    ...
    total_dropped = sum(
        f.get("dropped_packets", 0) or f.get("packet_drops", 0) for f in recent_flows
    )
    dropped_packets = (
        total_dropped if total_dropped > 0 else stats.get("dropped_packets", 0)
    )

    active_iocs = sum(f.get("active_iocs", 0) for f in recent_flows)
    if active_iocs == 0:
        active_iocs = stats.get(
            "active_iocs",
            len([f for f in recent_flows if f.get("is_ioc") or f.get("has_ioc")]),
        )

    malicious_blocked = sum(f.get("malicious_blocked", 0) for f in recent_flows)
    if malicious_blocked == 0:
        malicious_blocked = stats.get(
            "malicious_blocked",
            len(
                [
                    f
                    for f in recent_flows
                    if f.get("status") == "blocked" or f.get("blocked")
                ]
            ),
        )
    ...
    syn_packet_count = sum(
        f.get("syn_packets", 0) or f.get("syn_count", 0) for f in recent_flows
    )
    if syn_packet_count == 0:
        syn_packet_count = stats.get("syn_packet_count", stats.get("syn_count", 0))

    ack_packet_count = sum(
        f.get("ack_packets", 0) or f.get("ack_count", 0) for f in recent_flows
    )
    if ack_packet_count == 0:
        ack_packet_count = stats.get("ack_packet_count", stats.get("ack_count", 0))

    total_packets = sum(
        f.get("packet_count", 0) or f.get("packets", 0) for f in recent_flows
    )
    ...
    anomaly_flows = (
        [f for f in recent_flows if f.get("status") == "anomaly"]
        if recent_flows
        else []
    )
    ...
    if anomaly_flows:
        con = round(
            max(
                f.get("confidence", 0.0) or (f.get("score", 0.0) * 100)
